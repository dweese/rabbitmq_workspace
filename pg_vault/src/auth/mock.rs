...
struct MockState {
    ...
    secret_key: Vec<u8>,
    /// The "correct" PIN for this mock device
    correct_pin: String,
...
impl Default for MockState {
    fn default() -> Self {
        Self {
            ...
            correct_pin: "123456".to_string(), // Default PIN
            failure_rate: 0.0,
            auth_attempts: 0,
...
impl YubikeyAuth for MockYubikey {
    ...
    fn challenge_response(&self, challenge: &[u8], pin: &str) -> AuthResult<Vec<u8>> {
        let mut state = self
            .state
            .lock()
            .map_err(|_| AuthError::TokenError("Failed to lock mock state".to_string()))?;
        ...
        if challenge.len() > 255 {
            return Err(AuthError::AuthenticationFailed("Challenge too long".to_string()));
        }

        // Check the PIN
        if pin != state.correct_pin {
            return Err(AuthError::AuthenticationFailed("Invalid PIN".to_string()));
        }
        ...
        if state.simulate_slow_ops {
            ...
            state = self
                .state
                .lock()
                .map_err(|_| AuthError::TokenError("Failed to relock mock state".to_string()))?;
        }

        // Check for simulated failures
        if self.should_fail(state.failure_rate) {
            return Err(AuthError::AuthenticationFailed("Simulated failure".to_string()));
        }
...
mod tests {
    ...
    fn test_challenge_response() {
        ...
        let response = mock.challenge_response(challenge, "123456").unwrap();
        assert_eq!(response.len(), 20);

        // Same challenge should produce same response
        let response2 = mock.challenge_response(challenge, "123456").unwrap();
    ...
    fn test_different_challenges_different_responses() {
        ...
        let response1 = mock.challenge_response(b"challenge1", "123456").unwrap();
        let response2 = mock.challenge_response(b"challenge2", "123456").unwrap();
    ...
    fn test_not_present() {
        ...
        let result = mock.challenge_response(b"test", "123456");
    ...
    fn test_invalid_pin() {
        let mock = MockYubikey::new();
        let result = mock.challenge_response(b"test", "wrong-pin");
    ...
    fn test_with_touch() {
        ...
        mock.set_requires_touch(true); // Explicitly set for clarity

        // First attempt might require touch
        let _result = mock.challenge_response(b"test", "123456");
    ...
    fn test_empty_challenge() {
        ...
        let result = mock.challenge_response(&[], "123456");
    ...
    fn test_reset() {
        ...
        mock.challenge_response(b"test", "123456").unwrap();
        assert!(mock.auth_attempt_count() > 0);
