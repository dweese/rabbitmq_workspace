...
impl Vault {
    ...
    pub fn new_with_hardware(
        ...
        let auth_provider = Arc::from(AuthProviderFactory::create_provider_with_fallback(auth_config.clone()));
    ...
    pub async fn connect(&self) -> VaultResult<SecureConnection> {
        ...
        let response = self.auth_provider.challenge_response(&challenge)?;
        
        // Validate authentication response
        crate::auth::utils::validate_response(&response, Some(20))?; // HMAC-SHA1 is 20 bytes
    ...
    pub async fn connect_with_challenge(&self, challenge: &[u8]) -> VaultResult<SecureConnection> {
        // Perform authentication with provided challenge
        let response = self.auth_provider.challenge_response(challenge)?;
        crate::auth::utils::validate_response(&response, Some(20))?;
    ...
    pub async fn cleanup_expired_sessions(&self) {
        ...
        sessions.retain(|_id, session| {
            ...
            let lifetime_ok = now.duration_since(session.created_at)
                .unwrap_or(Duration::ZERO) < self.config.max_session_lifetime;
    ...
    fn generate_challenge(&self) -> Vec<u8> {
        crate::auth::utils::generate_challenge(self.config.challenge_length)
    }
