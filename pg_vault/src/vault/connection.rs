...
use tokio_postgres::{Client, Error as PgError, Row, Statement};
...
impl Connection {
    ...
    pub async fn query_prepared(
        ...
        let timeout_duration = Duration::from_secs(self.config.query_timeout);
        let result =
            tokio::time::timeout(timeout_duration, self.client.query(statement, params)).await;
        ...
        match result {
            ...
            Err(_) => {
                ...
                Err(VaultError::Timeout(format!(
                    "Query prepared timeout after {}s",
                    self.config.query_timeout
                )))
            }
        }
    ...
    pub async fn execute_prepared(
        ...
        let timeout_duration = Duration::from_secs(self.config.query_timeout);
        let result =
            tokio::time::timeout(timeout_duration, self.client.execute(statement, params)).await;
        ...
        match result {
            ...
            Err(_) => {
                ...
                Err(VaultError::Timeout(format!(
                    "Execute prepared timeout after {}s",
                    self.config.query_timeout
                )))
            }
        }
