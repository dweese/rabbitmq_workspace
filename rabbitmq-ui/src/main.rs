...
impl AppState {
    fn connect_to_rabbitmq(&mut self) {
    ...
    fn disconnect_from_rabbitmq(&mut self) {
    ...
    fn refresh_queues_and_exchanges(&mut self) {
        if let Some(client) = &self.client {
            let client = client.clone();
            let runtime = self.runtime.clone();
...
impl eframe::App for MyApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        // Create our own manual layout instead of using the BorderLayout
        ...
        for action in actions {
            match action {
                ...
                UiAction::DeclareQueue => {
                    self.state.status_message = "Queue declaration not implemented yet".to_string();
                },
                UiAction::DeclareExchange => {
                    self.state.status_message = "Exchange declaration not implemented yet".to_string();
                },
            }
        }
    }
